...
class RGBDCameraSubscriber(Node):
    def __init__(self, rgb_topic, camera_info_topic):
        ...
        self.target_poses = {0: None, 1: None, 2: None}  # Dictionary to store poses for ids 0, 1, 2
    ...
    def calculate_pose(self, rvec, tvec):
        ...
        # transform = self.tf_buffer.lookup_transform('base_link', 'base_footprint', rclpy.time.Time())
        # transformed_pose = tf2_geometry_msgs.do_transform_pose(transform_robot, transform)
        #
        # transform_robot = transformed_pose
    ...
    def save_target_poses(self):
        target_data = {}
        # Add the current target poses to the dictionary
        for marker_id, pose in self.target_poses.items():
            if pose is not None:
                target_data[str(marker_id)] = {
                    "position": pose[0].tolist(),
                    "orientation": pose[1]
                }

        # Read the existing target data from the file (if any)
        if os.path.exists(self.config_file):
            with open(self.config_file, 'r') as f:
                existing_data = json.load(f)
                # Update with new data, if any
                existing_data.update(target_data)
                target_data = existing_data  # The updated data to write back

        # Write the updated target poses back to the file
        with open(self.config_file, 'w') as f:
            json.dump(target_data, f)
        self.get_logger().info("Target poses saved or updated in the configuration file.")

    def load_target_poses(self):
        if os.path.exists(self.config_file):
            with open(self.config_file, 'r') as f:
                target_data = json.load(f)
                for marker_id, pose_data in target_data.items():
                    # Each pose is stored as a tuple (position, orientation)
                    self.target_poses[int(marker_id)] = (np.array(pose_data["position"]), pose_data["orientation"])
            self.get_logger().info("Target poses loaded from configuration file.")
        else:
            self.get_logger().info("No configuration file found. No target poses loaded.")
...
if __name__ == '__main__':
    ...
    qr_to_object_transform[:3, 3] = [50, 120, -501]  # x=50, y=120, z=501
                ...
                if len(ids) >= 1:
                    for i, id in enumerate(ids):
                        ...
                        if success:
                            ...
                            if rgbd_camera_subscriber.mode == 1 and rgbd_camera_subscriber.target_poses[id] is None and id in rgbd_camera_subscriber.target_poses:
                                rgbd_camera_subscriber.target_poses[id] = (position, orientation)
                                rgbd_camera_subscriber.get_logger().info(f"Target pose for marker {id} set.")

                            elif rgbd_camera_subscriber.mode == 2 and rgbd_camera_subscriber.target_poses.get(id) is not None:
                                target_position, target_orientation = rgbd_camera_subscriber.target_poses[id]
