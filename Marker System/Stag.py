...
class RGBDCameraSubscriber(Node):
    def __init__(self, rgb_topic, camera_info_topic):
        ...
        self.cam2robot = np.array([[-0.03138, -0.50278, 0.86385, 119.61536],
                            [-0.99951, 0.01691, -0.02647, 2.27759],
                            [-0.0013, -0.86425, -0.50306, 205.82016],
                            [0.0, 0.0, 0.0, 1.0]])
        ...
        self.publisher_qr = self.create_publisher(Image, 'baoma_qr', 10)
        ...
        self.position_queue = collections.deque(maxlen=5)
        self.orientation_queue = collections.deque(maxlen=5)
    ...
    #         avg_orientation = np.mean(np.array(self.orientation_queue), axis=0)
    #
    #         # Calculate the max and min values for position and orientation queues
    #         position_range = np.max(np.array(self.position_queue), axis=0) - np.min(np.array(self.position_queue), axis=0)
    #         orientation_range = np.max(np.array(self.orientation_queue), axis=0) - np.min(np.array(self.orientation_queue), axis=0)
    #
    #         # Define a threshold for the acceptable range of position and orientation errors
    #         position_threshold = 5  # Adjust this value based on your system's tolerance
    #         orientation_threshold = 0.05  # Adjust this value based on your system's tolerance
    #
    #         # Only publish the pose if the range of positions and orientations is within the threshold
    #         if np.all(position_range < position_threshold) and np.all(orientation_range < orientation_threshold):
    #             self.publish_pose(avg_position.tolist(), avg_orientation.tolist())
    #         else:
    #             self.get_logger().info("Pose smoothing skipped due to large error.")
    ...
    def publish_image_qr(self, img):
        # 读取图像
        # 将 OpenCV 图像转换为 ROS 消息格式
        ros_image = self.bridge.cv2_to_imgmsg(img, encoding="bgr8")

        # 发布图像消息
        self.publisher_qr.publish(ros_image)
...
if __name__ == '__main__':
    ...
    qr_to_object_transform[:3, 3] = [20, 105, -460]   # x=50, y=120, z=479
    ...
    try:
        while rclpy.ok():
            if rgbd_camera_subscriber.mtx is not None and rgbd_camera_subscriber.rgb is not None:
                ...
                stag.drawDetectedMarkers(rgbd_camera_subscriber.rgb, corners, ids)
                ...
                if len(ids) >= 1:
                    for i, id in enumerate(ids):
                        ...
                        if success:
                            cv2.drawFrameAxes(rgbd_camera_subscriber.rgb, rgbd_camera_subscriber.mtx, rgbd_camera_subscriber.dist, rvec, tvec, 50) # 50 is the axis length in pixels
                            rgbd_camera_subscriber.publish_image_qr(rgbd_camera_subscriber.rgb)
                ...
                rgbd_camera_subscriber.rgb = None
