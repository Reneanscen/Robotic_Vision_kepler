...
# 将一个抓取的三维点投影到二维图像上，并在图像上绘制抓取的可视化线条
...
def detect_label(image, center_point, ShelfWorldCoor, K, cam2robot):
    ...
    # 货架在世界坐标系下的位置
    # ShelfWorld
    # Coor = np.array([[0, 0, 0],[327, 0, 0],[327, 380, 0],[0, 380, 0]], float)
    # qr中心点坐标
...
class ObjectDection(Node):
    def __init__(self):
        super().__init__('kepler_vision')
        self.system_command = [1, 2, 2]  # [开关， 行, 列]
        # 订阅
        # 和系统控制对接
        self.subscription = self.create_subscription(Int32MultiArray, '/perception/switch', self.manage_callback, 10)
        # 图像
        self.camera_info_subscriber = self.create_subscription(CameraInfo, '/camera/color/camera_info', self.callback_intrinsics, 30)
        self.rgb_subscriber = self.create_subscription(Image, '/camera/color/image_raw', self.callback_rgb, 30)
        
        # 初始化qr码检测类
        self.i_QRDetect = QRDetect()

        self.freq_sample = 20  # running frequency: 5 or 10 Hz is recommanded
        self.result_volume = 10 # 结果缓存大小
        self.timer = self.create_timer(1 / self.freq_sample, self.perception_callback)

        # 定义发布可视化结果的话题
        self.pub_img = self.create_publisher(Image, '/perception/object_detection/display_img', self.freq_sample)
        self.pub_img_table = self.create_publisher(Image, '/perception/object_detection/display_img_table', self.freq_sample)

        # 定义发布数据结果的话题
        self.pub_arm_pose = self.create_publisher(ArmPoseArray, '/perception/object_detection/arm_pose', self.freq_sample)
        self.pub_table_pose = self.create_publisher(ArmPoseArray, '/perception/object_detection/table_pose', self.freq_sample)

        # 各种目标的世界坐标系(mm)
        self.qr_world = np.array([[0, 0, 0],[58.6, 0, 0],[58.6, 57, 0],[0, 57, 0]], float)              # qr码世界坐标系
        self.box_world = np.array([[60,-157,190], [60,157,190], [60,157,-25], [60,-157,-25]], float)    # 箱子世界坐标系
        self.grasps_box_left = np.array([[-105,-160,0], [50, 50, 0]])                                   # 左钩取点偏移量
        self.grasps_box_right = np.array([[-105,160,0], [50, -50, 0]])                                  # 右钩取点偏移量
        self.ShelfWorldCoor = np.array([[0, 0, 0],[380, 0, 0],[380, 205, 0],[0, 205, 0]], float)        # 货架的世界坐标系
        self.TableWorldLabel = np.array([[0, 0, 0],[0, 0, -200],[0, 449, -200],[0, 449, 0]], float)     # 桌子上标签的点的世界坐标系

        markerSize = 277
        self.WC_TableQR = np.array([[0, 0, 0],[0, 0, markerSize],[0, markerSize, markerSize],[0, markerSize, 0]], float)       # 桌子上的QR码的世界坐标系
        ...
        self.cam2robot = np.array([[-0.0594, -0.48209, 0.8741, 123.7577], [-0.99802, 0.01067, -0.06194, 10.66395],
  [0.02053, -0.87605, -0.48177, 209.17038], [0.0, 0.0, 0.0, 1.0]])
        ...
        # 预设高度辅助检测精度
        self.rows_z = np.array([[360], [110], [-300], [-600], [-850]], float)   # 每行qr码在机器人坐标系的高度
        self.shelf_row_z = np.array([[360-150], [110-150], [-300-150], [-600-150], [-850-150]], float)  # 货架每行货架右下角耳在机器人坐标系下的高度
        self.grasp_row_z = np.array([[-99999], [54], [-326], [-646], [-99999]], float)  # 钩取点的固定高度
    ...
    def clean_buffer(self):
        self.lift_xyz_buffer = deque(maxlen=self.result_volume)             # 右钩取点位置
        self.lift_xyz_buffer_left = deque(maxlen=self.result_volume)        # 左钩取点位置
        self.lift_rz_buffer = deque(maxlen=self.result_volume)              # 钩取点yaw角
        self.carry_xyz_buffer_left = deque(maxlen=self.result_volume)       # 左抱取点位置
        self.carry_xyz_buffer_right = deque(maxlen=self.result_volume)      # 右抱取点位置
        self.carry_rz_buffer = deque(maxlen=self.result_volume)             # 抱取点yaw角
        self.shelf_xyz_buffer = deque(maxlen=self.result_volume)            # 货架位置
        self.shelf_rz_buffer = deque(maxlen=self.result_volume)             # 货架角度
        self.box_pull_buffer = deque(maxlen=self.result_volume)             # 箱子钩出距离
        self.table_xyz_buffer = deque(maxlen=self.result_volume)            # 桌子位置
        self.table_rz_buffer = deque(maxlen=self.result_volume)             # 桌子yaw角

    def manage_callback(self, msg):
        self.system_command = msg.data
    ...
    def perception_callback(self):
        ...
        if len(yolo_output) > 0:
            ...
            if len(cls_names) == len(kpts):
                ...
                kpts = kpts[idx_conf]               # 关键点
                cls_names = cls_names[idx_conf]     # 类别名
                boxes = boxes[idx_conf]             # bbox
                ...
                # 根据系统指定的行列格口，从检测值中找到此格口
                # 如果系统发的是偶数列（0开始数），且红色结果数大于0
                if (col_assign%2 == 0) and 0 < len(shelfRed_kpts):
                    ...
                    if np.isin(row_shelf, pose_rowno):
                        ...
                        # 待发送
                        pose_assign[1, 3] = pose_assign[1, 3] - 90  # 将检测到的格口右下角的y转为格口中心的y
        ...
        print(f"执行一帧总耗时: {elapsed_time} 秒")

        ##########
        # 2024.12.23临时配合机械臂调试  调完注释
        # 勾取点设置固定值
        ...
        # 6. 日至打印
        # 左侧够取点/右侧够取点/左侧抱取点/右侧抱取点/货架/拉取距离
        ...
        # 发送
        arm_pose_array = ArmPoseArray()
        arm_pose_array.arm_poses = [ap1, ap2, ap3, ap4]  # 将 ArmPose 对象列表赋值给 arm_poses 字段

        # 发送消息
        self.pub_arm_pose.publish(arm_pose_array)
        ...
        cv2.imwrite(CODE_DIR + 'rgb_table.jpg', rgb_table)



if __name__ == '__main__':
    
    ASSETS = '/home/kepler/Spatial_AI/perception_20250207/src/assets/'
    yolo = YOLO(ASSETS + 'yolov8s-pose_shanghai_v2.pt') #

    rclpy.init()

    object_detection = ObjectDection()
